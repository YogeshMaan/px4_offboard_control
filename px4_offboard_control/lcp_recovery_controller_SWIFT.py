...
class AdmittanceCollisionDetection(Node):
    ...
    def generateWaypoints(self):
        ...
        # Wp 2
        wp.append([0.35 , 0.53, -0.8, 0.0 , 0.0,float("nan"), self.yaw])
        # Wp 3
        wp.append([0.35 , 0.53, -0.8, float("nan"),float("nan"),float("nan"), self.yaw])  # Velocities depend on angle of collision and ADD Yaw
        # Wp 4 (modified after collision)
        wp.append([0.35 , 0.53, -0.8, float("nan"),float("nan"),float("nan"), self.yaw])
        # Wp 5 (after mission over)
        wp.append([0.35 , 0.23, -0.8, float("nan"),float("nan"),float("nan"), self.yaw])
        # Wp 6 (landing wp)
        wp.append([0.35 , 0.23, 0.0, float("nan"),float("nan"),float("nan"), self.yaw])
    ...
    def check(self):        
        ...
        if self.wp_num == 3:
            if self.euclideanError(self.wp_num) >= self.thres_error:
                ...
                self.wp_num = 3 # keep it hovering over the modified waypoint
