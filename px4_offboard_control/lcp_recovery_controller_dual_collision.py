...
class AdmittanceCollisionDetection(Node):
    ...
    def generateWaypoints(self):
        ...
        wp = []
        # Wp 0
        wp.append([0.0 , 0.0, -.75, float("nan"), float("nan"),float("nan"), self.yaw])
        # Wp 1 (modified after collision 1)
        wp.append([0.0, 0.0, -.75, float("nan"),float("nan") ,float("nan"), self.yaw])
        # Wp 2 (modified after collision 2)
        wp.append([0.0, 0.0, -.75, float("nan"),float("nan"),float("nan"), self.yaw])  # Velocities depend on angle of collision and ADD Yaw
    ...
    def engage_offboard_mode(self):
        ...
        self.get_logger().info('Switching to offboard mode, yep3!')
    ...
    def check(self):        
        ...
        if self.wp_num == 1:
            ...
            if not self.collision_status and self.delta_t <= self.thres_delta_t:
                self.wp_publisher(self.wp_num)
                self.get_logger().info(f"Ready to take hit!: Time left-{self.delta_t-self.thres_delta_t}, wp num: {self.wp_num}")
            elif not self.collision_status and self.delta_t > self.thres_delta_t:
                self.wp_num = 4 #hover over landing point
                self.wp_publisher(self.wp_num)
        ...
        if self.wp_num == 3:
            if self.euclideanError(self.wp_num) >= self.thres_error:
                ...
                self.wp_num -= 1 # to send back to original hovering wp under admittance
                self.wp_publisher(self.wp_num)
    ...
    def modifyWaypoint4(self):
        ...
        self.waypoints[3] = [x_new, y_new, -.75, float("nan"),float("nan"),float("nan"), self.yaw] # Do it using modifyWaypoint4()
    ...
    def checkCollision(self):
        ...
        if max_acc > self.thresh_acc or min_acc < -self.thresh_acc:
